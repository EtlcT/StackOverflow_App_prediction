...
class BaseTextCategorizationDataset:
    ...
    def get_label_to_index_map(self):
        ...
        #? DONE
        label_to_index = dict()
        for key, value in self.get_index_to_label_map().items():
            label_to_index[value] = key
    ...
    def to_indexes(self, labels):
        ...
        indexes = []
        for label in labels :
            indexes.append(self.get_index_to_label_map()[label])
        return indexes
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        try:
            ...
            print('An error occured while reading csv')
            raise

        # assert that columns are the ones expected
        # TODO: CODE HERE
        #? DONE
        assert(dataset.columns.to_list() == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title'])
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                return df.groupby('tag_id').filter(lambda y: len(y)>=x)
            return filter_function
